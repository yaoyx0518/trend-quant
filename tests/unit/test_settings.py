...
class TestLoadSettings:
    ...
    def test_default_values(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            config_path = Path(tmp) / "app.yaml"
            config_path.write_text(
                yaml.dump({"app": {}, "tickflow": {}, "runtime": {}, "logging": {}}),
                encoding="utf-8",
            )
            settings = load_settings(config_path)
            assert settings.app.port == 8000
            assert settings.app.host == "127.0.0.1"
            assert settings.runtime.account_equity_default == 200000
    ...
    def test_app_settings_defaults(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            settings = load_settings(config_path)
            assert settings.app.lot_size == 100
            assert settings.tickflow.plan == "starter"
            assert settings.runtime.ensure_dirs is True
